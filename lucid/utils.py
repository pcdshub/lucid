...
SEARCH_PATTERN = re.compile(r"((?P<category>[a-z_][a-z0-9_]*):\s*)?(?P<text>[^ ]+)", re.VERBOSE | re.IGNORECASE)
...
def split_search_pattern(text):
    ...
    matches = [m.groupdict() for m in SEARCH_PATTERN.finditer(text.strip())]
    by_category = [(m["category"], m["text"]) for m in matches if m["category"] is not None]

    general = [m["text"] for m in matches if m["category"] is None]
...
def get_happi_device_cache():
    ...
    if _HAPPI_CACHE is None or (time.monotonic() - _HAPPI_CACHE[0]) > HAPPI_CACHE_UPDATE_PERIOD:
...
def screenshot_top_level_widgets():
    ...
    for screen_idx, screen in enumerate(app.screens(), 1):  # type: ignore
        logger.debug("Screen %d: %s %s", screen_idx, screen, screen.geometry())

    primary_screen = app.primaryScreen()  # type: ignore
    ...
    index = 0
    for widget in sorted(app.topLevelWidgets(), key=by_title):  # type: ignore
    ...
    *,
    ...
    if save_screenshots:
        try:
            ...
            screenshots = "\n".join(
                f'screenshot{idx}="{screenshot_fn}"' for idx, screenshot_fn in enumerate(screenshot_files, 1)
            )
            message = f"{message}\nscreenshot_id={screenshot_id}\n{screenshots}"

    pcdsutils.log.logger.log(level, message, exc_info=exc_info, stacklevel=stacklevel + 1)
...
def get_fully_qualified_domain_name():
    ...
    try:
        ...
        logger.warning("Unable to get machine name.  Things like centralized logging may not work.")
