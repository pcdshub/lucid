...
def get_parser():
    ...
    parser.add_argument(
        "--version", action="version", version=f"LUCID {__version__}", help="Show LUCID's version number and exit."
    )

    parser.add_argument("beamline", help="Specify the beamline name to compose the home screen.", type=str, nargs="+")
    parser.add_argument(
        "--toolbar",
    ...
    parser.add_argument("--skip_happi", help="Skip loading entries from happi", action="store_true", default=False)
    return parser
    ...
    def _load_demo(self):
        ...
        for stand in ("DIA", "DG1", "TFS", "DG2", "TAB", "DET", "DG3"):
            for system in ("Timing", "Beam Control", "Diagnostics", "Motion", "Vacuum"):
                ...
                devices = [SynAxis(name=f"{stand.lower()}_{system_name}_{i}") for i in range(device_count)]
...
def launch(
    ...
    *,
    ...
    window = lucid.main_window.LucidMainWindow(beamline=beamline, toolbar=toolbar)
    ...
    def handle_error(exc_info):
        ...
        logger.exception("An uncaught exception happened: %s", exc_value, exc_info=exc_info)
